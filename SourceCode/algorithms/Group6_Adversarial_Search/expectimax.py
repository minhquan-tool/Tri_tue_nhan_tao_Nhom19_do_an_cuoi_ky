...
def solve_firefighter_expectimax(matrix, start_pos, victims, gates):
    """
    Expectimax Search: Xử lý nút môi trường như một nút xác suất (Chance Node).
    """
    ...
    def get_cost(r, c):
        ...
        if matrix[r][c] == 2: return 5 
        return 1

    def manhattan(p1, p2):
        return abs(p1[0] - p2[0]) + abs(p1[1] - p2[1])

    def find_path_expectimax(start, target):
        queue = [(start, [start], 0)]
        visited = {start}
    ...
    while unrescued:
        # 1. Cứu nạn nhân gần nhất
        unrescued.sort(key=lambda v: manhattan(current_pos, v))
        target_victim = unrescued[0]
        
        path_to_v, cost_to_v = find_path_expectimax(current_pos, target_victim)
        if not path_to_v: 
            stats['reason'] = f"Đường đến nạn nhân {target_victim} bất khả thi."
        ...
        # 2. Thoát hiểm ra cửa gần nhất
        gates.sort(key=lambda g: manhattan(current_pos, g))
        target_gate = gates[0]
        
        path_to_g, cost_to_g = find_path_expectimax(current_pos, target_gate)
        if not path_to_g: 
            stats['reason'] = f"Bị kẹt! Không thể đến cửa thoát hiểm từ {current_pos}"
            return rescue_order, full_path, total_cost, stats
